...
template <typename K, typename V> class HashMapaLan : public Mapa<K, V> {
...
public:
  ...
  void definisiHashFunkciju(function<unsigned int(const K &, unsigned int)> h) {
    this->h = h;
  }
};
...
template <typename K, typename V>
typename list<pair<K, V>>::const_iterator
HashMapaLan<K, V>::Trazi(const K &kljuc) const {
  if (!h)
    throw "Hash funkcija nije definisana";
  int hes(h(kljuc, Velicina()));
  auto it(mapa[hes].begin());
...
template <typename T> class Grana;
template <typename T> class Cvor;
template <typename T> class GranaIterator;

template <typename T> class UsmjereniGraf {
protected:
...
public:
  ...
  const GranaIterator<T> dajGraneKraj() const {
    return GranaIterator<T>(const_cast<UsmjereniGraf *>(this), dajBrojCvorova(),
                            0);
  }
};
...
public:
  Grana(UsmjereniGraf<T> *graf, int polazni, int dolazni)
...
public:
  Cvor(UsmjereniGraf<T> *graf, int rbroj) : graf(graf), rbroj(rbroj) {}
...
template <typename T> GranaIterator<T> &GranaIterator<T>::operator++() {
  do
    inkrementuj(i, j);
...
template <typename T> GranaIterator<T> GranaIterator<T>::operator++(int) {
  auto kopija(*this);
  ++(*this);
  return kopija;
}

template <typename T> class ListaGraf : public UsmjereniGraf<T> {
  struct Element {
    int cvor;
    float tezina;
    T oznaka;
...
template <typename T>
typename list<typename ListaGraf<T>::Element>::iterator
ListaGraf<T>::pronadjiElement(int i, int j) {
...
template <typename T>
typename list<typename ListaGraf<T>::Element>::const_iterator
ListaGraf<T>::pronadjiElement(int i, int j) const {
...
template <typename T> void ListaGraf<T>::validirajCvor(int indeks) const {
  if (indeks < 0 || indeks >= dajBrojCvorova())
    throw "Neispravan broj cvora";
}

template <typename T> void ListaGraf<T>::validirajGranu(int i, int j) const {
  validirajCvor(i);
  validirajCvor(j);
}
...
template <typename T> ListaGraf<T>::ListaGraf(int brojCvorova) {
  if (brojCvorova < 0)
...
template <typename T> void ListaGraf<T>::dodajGranu(int i, int j, float w) {
  validirajGranu(i, j);
  typename list<Element>::iterator it;
  for (it = listaSusjedstva[i].begin();
       it != listaSusjedstva[i].end() && j > it->cvor; it++)
    ;
  listaSusjedstva[i].insert(it, Element(j, w));
}

template <typename T> void ListaGraf<T>::obrisiGranu(int i, int j) {
...
template <typename T> float ListaGraf<T>::dajTezinuGrane(int i, int j) const {
  validirajPostojanjeGrane(i, j);
  return pronadjiElement(i, j)->tezina;
}

template <typename T> bool ListaGraf<T>::postojiGrana(int i, int j) const {
  validirajGranu(i, j);
  return pronadjiElement(i, j) != listaSusjedstva[i].end();
}

template <typename T> void ListaGraf<T>::postaviOznakuCvora(int i, T oznaka) {
  validirajCvor(i);
  oznakeCvorova[i] = oznaka;
}

template <typename T> T ListaGraf<T>::dajOznakuCvora(int i) const {
  validirajCvor(i);
...
template <typename T> T ListaGraf<T>::dajOznakuGrane(int i, int j) const {
  validirajPostojanjeGrane(i, j);
  return pronadjiElement(i, j)->oznaka;
}

template <typename T> Grana<T> ListaGraf<T>::dajGranu(int i, int j) {
  validirajPostojanjeGrane(i, j);
...
template <typename T> Cvor<T> ListaGraf<T>::dajCvor(int i) {
  validirajCvor(i);
  return Cvor<T>(this, i);
}

template <typename T> const Cvor<T> ListaGraf<T>::dajCvor(int i) const {
  validirajCvor(i);
...
void dfs(UsmjereniGraf<TipOznake> *graf, vector<Cvor<TipOznake>> &dfs_obilazak,
  ...
  for (GranaIterator<TipOznake> it = graf->dajGranePocetak();
